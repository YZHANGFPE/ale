 ...
 *  ram_agent_ql.cpp
 *
 *  The implementation of the RAMAgentQL class, which is a generic player 
...
RAMAgentQL::RAMAgentQL(GameSettings* _game_settings, OSystem* _osystem) :
 ...
 ******************************************************************** */
Action RAMAgentQL::agent_step(const IntMatrix* screen_matrix, 
	...
	if (b_end_episode_with_reward && b_reward_on_this_frame) {
		...
		p_ql_lambda_solver->episode_end(f_curr_reward, f_curr_reward);
	...
	if (e_episode_status == ACTION_EXPLOR) {
		next_action_ind = p_ql_lambda_solver->episode_step(
                                                   pv_curr_feature_map, 
                                                   pv_num_nonzero_in_f,
                                                   f_curr_reward, 
...
******************************************************************** */
Action RAMAgentQL::on_start_of_game(void) {
    generate_feature_vec();
    assert((*pv_num_nonzero_in_f)[0] > 0);
	int next_action_ind;
	if (e_episode_status == ACTION_EXPLOR) {
		next_action_ind = p_ql_lambda_solver->episode_start(
 ...
 ******************************************************************** */
void RAMAgentQL::on_end_of_game(void) {
 ...
 ******************************************************************** */
void RAMAgentQL::generate_feature_vec(void) {
    for (int a = 0; a < i_num_actions; a++) {
