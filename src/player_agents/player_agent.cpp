...
PlayerAgent::PlayerAgent(GameSettings* _game_settings, OSystem* _osystem) : 
	...
	string suffix = _osystem->settings().getString("suffix");
    int len = rom_file.length() - 11;
    string rom_name = rom_file.substr(7,len);

    string dir = "./output/" + player_agent + "_" + rom_name + "_" + suffix + "/";
    ...
    _osystem->settings().setString("prefix", dir);
    prefix = dir;
}
...
void PlayerAgent::on_end_of_game(void) {
    ...
    if ((i_export_rewards_frq != 0) &&
		...
		filename.str("");
		filename << prefix << "reward_per_frame__frame_0_" << i_frame_counter << ".txt";
		export_vector(pv_reward_per_frame, filename.str());
		filename.str("");
		filename << prefix << "epis_start_frame__epis_0_" << i_episode_counter << ".txt";
		export_vector(pv_episodes_start_frame, filename.str());
		filename.str("");
		filename << prefix << "epis_end_frame__epis_0_" << i_episode_counter << ".txt";
...
PlayerAgent* PlayerAgent::generate_agent_instance(
    ...
    } else if (player_agent == "ram_agent_ql") {
        cout << "Game will be controlled by RAM Agent (Q-learning)" << endl;
