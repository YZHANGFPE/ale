...
def plot_rewards_per_episode(file, agent_name, game_name, delimiter = ',', num_steps = 1):
    ...
    inline_split = inline[0].split(delimiter)
    x = []
    ...
    max_val = max(walking_avg_x)
    min_val = min(walking_avg_x)
    ...
    plt.plot(walking_avg_x, label=agent_name)
    plt.plot((0, max_val * 1.1))
    plt.plot((0, min_val - 0.1))
    plt.legend()
